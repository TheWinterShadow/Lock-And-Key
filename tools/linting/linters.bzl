...
def _make_lint_check(name, entry_point, deps):
    if entry_point == "black":
        cmd_command = "$(location //tools/linting:runner) {} --check --diff $(locations //lock_and_key:all_py_files) > $@ 2>&1 && echo '{}: No formatting issues' > $@ || (echo '{} formatting issues found:' && $(location //tools/linting:runner) {} --check --diff $(locations //lock_and_key:all_py_files) 2>&1 | tee $@ && exit 1)".format(entry_point, name.title(), name.title(), entry_point)
    elif entry_point == "isort":
        cmd_command = "$(location //tools/linting:runner) {} --check-only --diff $(locations //lock_and_key:all_py_files) > $@ 2>&1 && echo '{}: No import issues' > $@ || (echo '{} import sorting issues found:' && $(location //tools/linting:runner) {} --check-only --diff $(locations //lock_and_key:all_py_files) 2>&1 | tee $@ && exit 1)".format(entry_point, name.title(), name.title(), entry_point)
