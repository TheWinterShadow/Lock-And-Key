...
class TestAWSProvider(unittest.TestCase):
    ...
    @patch(
        "lock_and_key.providers.aws.aws_policy_collector.AWSPolicyCollector.scan_all_policies"
    )
    @patch("lock_and_key.providers.aws.resources.iam.IAMService.from_creds")
    def test_run_analysis_with_mocked_services(
        self, mock_iam_from_creds, mock_collector_scan
    ):
